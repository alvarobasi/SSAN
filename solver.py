...
def main(args):
    ...
    # define model
    # model = get_model(args.model_type, max_iter).to(device)
    # def optimizer
    ...
    for epoch in range(args.start_epoch, args.num_epochs):
        ...
        # train
        # model.train()
        for i, (frames, labels, uuids) in enumerate(train_loader):
            frames, labels, uuids = frames.to(device), labels.to(device), uuids.to(device)
            if args.model_type in ["SSAN_R"]:
                rand_idx = torch.randperm(frames.shape[0])
                cls_x1_x1, fea_x1_x1, fea_x1_x2, domain_invariant = model(frames, frames[rand_idx, :, :, :])
                binary_loss = binary_fuc(cls_x1_x1, labels.long())
                ...
                adv_loss = binary_fuc(domain_invariant, uuids.long())
        ...
        if epoch % epoch_test == epoch_test-1:
            if args.trans in ["o", "p"]:
                test_data_dic = data_bank.get_datasets(train=False, protocol=args.protocol, img_size=args.img_size, map_size=args.map_size, transform=transformer_test_video(), debug_subset_size=args.debug_subset_size)
            ...
            for i, test_name in enumerate(test_data_dic.keys()):
                ...
                HTER, auc_test = test_video(model, args, test_loader, score_path, epoch, device, name=test_name)
...
def test_video(model, args, test_loader, score_root_path, epoch, device, name=""):
    model.eval()
    with torch.no_grad():
        start_time = time.time()
        scores_list = []
        for i, sample in enumerate(test_loader):
            frames, label = sample['image_x'].to(device), sample['label'].to(device)
            map_score = 0
            for frame_i in range(frames.shape[1]):
                if args.model_type in ["SSAN_R"]:
                    cls_x1_x1, fea_x1_x1, fea_x1_x2, _ = model(frames[:,frame_i,:,:,:], frames[:,frame_i,:,:,:])
                    score_norm = torch.softmax(cls_x1_x1, dim=1)[:, 1]
                elif args.model_type in ["SSAN_M"]:
                    pred_map, fea_x1_x1, fea_x1_x2, _ = model(frames[:,frame_i,:,:,:], frames[:,frame_i,:,:,:])
                    score_norm = torch.sum(pred_map, dim=(1, 2))/(args.map_size*args.map_size)
                map_score += score_norm
            map_score = map_score/frames.shape[1]
